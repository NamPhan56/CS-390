...
import edu.umass.cs.client.widget.ContextGraphWidget;
...
public class ContextActivity extends ListActivity {
	...
	private float prevCalorieCount = 0;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
	            ...
	            case Context_Service.MSG_ACCEL_VALUES:
	            {
	            	...
	            	//float speedDev = msg.getData().getFloat("speedDev");
	        	...
	        	//********************************************************************************
	            case Context_Service.MSG_SPEED_VALUES:
	            {
	            	Log.d(TAG,"got message");
	            	float speed = msg.getData().getFloat("speed");
	            	Log.d(TAG,"speed:" + speed);
	            	//int state = getStateFromActivityString(activity);
	            	if (widgets[STREAMS.speed.ordinal()] !=null){
	            		((ContextImageWidget)widgets[STREAMS.speed.ordinal()]).history_view.add(speed);
	            		((ContextImageWidget)widgets[STREAMS.speed.ordinal()]).setDescription("Your Speed: "+speed+"m/s");
	            		((ContextImageWidget)widgets[STREAMS.speed.ordinal()]).drawDescription();
	            	}
	            	break;
	            }
	            case Context_Service.MSG_SPEECH_STATUS:
	            {
	            	...
	            	if (widgets[STREAMS.speech.ordinal()] !=null){
	            		((ContextImageWidget)widgets[STREAMS.speech.ordinal()]).history_view.add(speech*10);
	            	}
	            	break;
	            }
	        	//********************************************************************************
	            ...
	            {
            		float stepCount = msg.getData().getFloat("stepCount");
	            	Log.d(TAG,"got message");
	            	float calorieCount = (float) Math.floor(stepCount/20);
	            	Context_Service.step_history.add(stepCount);
	            	Context_Service.calorie_graph_history.add((int) calorieCount);
	            	//http://www.livestrong.com/article/320124-how-many-calories-does-the-average-person-use-per-step/ 
	            	//says that an average of 1 calorie per step is burned
	            	Log.d(TAG,"Calorie Count:" + calorieCount);
	            	if (widgets[STREAMS.calorieTracker.ordinal()] !=null && calorieCount>prevCalorieCount){
	            		((ContextImageWidget)widgets[STREAMS.calorieTracker.ordinal()]).history_view.add(calorieCount);
	            		((ContextImageWidget)widgets[STREAMS.calorieTracker.ordinal()]).setDescription("Calorie Count: "+calorieCount);
	            		((ContextImageWidget)widgets[STREAMS.calorieTracker.ordinal()]).drawDescription();
	            		if(calorieCount>3){ //don't show the activity images
	            			((ContextImageWidget)widgets[STREAMS.calorieTracker.ordinal()]).setImage((int)calorieCount);//shows what food is equal to calories burned
	            		}
	            	}
	            	if (widgets[STREAMS.calorieGraph.ordinal()] !=null){
	            		((ContextGraphWidget)widgets[STREAMS.calorieGraph.ordinal()]).update(Context_Service.calorie_graph_history, Context_Service.step_history);
	            	}
	            	prevCalorieCount = calorieCount;
	            	break;
	            }
	        	//********************************************************************************
	            default:
...
//        view.setBackgroundColor(Color.WHITE);
    ...
    private void drawWidgets(){
    	...
    	for(int i : selected){
    		...
    		//**********************************************************************
    			case calorieTracker:
    				 if (Context_Service.calorie_history == null) 
     			   		Context_Service.calorie_history = new LinkedList<Float>();
     			   	widgets[i] = new ContinuousContextImageWidget(this,-500,500,500,Context_Service.calorie_history);
     			   	widgets[i].setTitle("Calories Burned to Food Consumed:");
     			   	widgets[i].addOrRemoveTitleViewAsNecessary();
     				break;
    		//**********************************************************************
    			case calorieGraph:
   				 if (Context_Service.calorie_graph_history == null) 
    			   		Context_Service.calorie_graph_history = new LinkedList<Integer>();
    			   	widgets[i] = new ContextGraphWidget(this,2,Context_Service.calorie_graph_history);
    			   	widgets[i].setTitle("            Calories Vs. Steps:");
    			   	widgets[i].addOrRemoveTitleViewAsNecessary();
    				break;
    		//**********************************************************************
	    		case speed:
				    if (Context_Service.speed_history == null) 
				   		Context_Service.speed_history = new LinkedList<Float>();
				   	widgets[i] = new ContinuousContextImageWidget(this,-20,20,150,Context_Service.speed_history);
				   	widgets[i].setTitle(STREAMS.speed.toString());
				   	widgets[i].addOrRemoveTitleViewAsNecessary();
					break;
			//**********************************************************************
	    		case speech:
				    if (Context_Service.speech_history == null) 
				   		Context_Service.speech_history = new LinkedList<Float>();
				   	widgets[i] = new ContinuousContextImageWidget(this,-20,20,150,Context_Service.speech_history);
				   	widgets[i].setTitle("Voice Stream");
				   	widgets[i].addOrRemoveTitleViewAsNecessary();
					break;
			//**********************************************************************
    			case ACTIVITY:
