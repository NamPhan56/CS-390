...
public class Context_Service extends Service implements SensorEventListener{
	...
	private static final double PEAKMAX = 10;
	private static final double PEAKMIN = 5;
	private static final double INTERVALMAX = 100;
	private static final double INTERVALMIN = 10;
	private double accelMax = 0;
	private double accelLast;
	private double accelMaxPrevious = 0;
	...
	public int detectSteps(double filt_acc_x, double filt_acc_y, double filt_acc_z) {
		...
		int ret = 0;
		int state = 0;
		accelTri = Math.sqrt(Math.pow(x,2) + Math.pow(y, 2) + Math.pow(z, 2));
		changeCount++;
		if (accelTri >= THRESHOLD) {
			accelMax = Math.max(accelMax, accelTri);
			state = 1;
			
		}
		else if (accelTri < THRESHOLD && accelLast >= THRESHOLD) {
			accelMaxPrevious = accelMax;
			accelMax = 0;
		}
		
		if (state == 1 && statePrevious == 0 && changeCount > 0) {
			if (isValidStep(changeCount, accelMaxPrevious)) {
				ret += 1;
			}
			changeCount = 0;
		}
	...
	private boolean isValidStep(int changeCount, double accelMaxPrevious) {
		return (changeCount >= INTERVALMIN && changeCount <= INTERVALMAX && 
				accelMaxPrevious <= PEAKMAX && accelMaxPrevious >= PEAKMIN);	
	}
